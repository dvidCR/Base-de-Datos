...
SELECT o.ciudad, o.pais, o.region
FROM cliente c
INNER JOIN empleado e
ON c.codigo_empleado_rep_ventas = e.codigo_empleado
INNER JOIN oficina o
ON e.codigo_oficina = o.codigo_oficina
WHERE o.ciudad = "Barcelona"; -- Fuenlabrada no existe asique te pongo Barcelona para que veas que funciona
...
SELECT e.nombre AS nombre_empelado, e.apellido1, e.apellido2, j.nombre AS nombre_jefe, j.apellido1, j.apellido2
FROM empleado e
INNER JOIN empleado j
ON e.codigo_empleado = j.codigo_jefe;

-- 9. Devuelve un listado que muestre el nombre de cada empleados, el nombre de su jefe y el nombre del jefe
-- de sus jefe.

SELECT e.nombre AS nombre_empelado, e.apellido1, e.apellido2, j.nombre AS nombre_jefe, j.apellido1, j.apellido2, j2.nombre AS nombre_jefe_del_jefe, j2.apellido1, j2.apellido2
FROM empleado e
INNER JOIN empleado j
ON e.codigo_empleado = j.codigo_jefe
RIGHT JOIN empleado j2
ON j.codigo_empleado = j2.codigo_jefe;
...
SELECT e.nombre, e.apellido1, e.apellido2, o.ciudad, o.pais, o.region, o.codigo_postal, o.telefono, o.linea_direccion1, o.linea_direccion2
FROM empleado e
LEFT JOIN cliente c
ON e.codigo_empleado = c.codigo_empleado_rep_ventas
...
SELECT pr.nombre, MAX(p.total), MIN(p.total)
FROM producto pr
INNER JOIN detalle_pedido dp
ON pr.codigo_producto = dp.codigo_producto
INNER JOIN pedido pe
...
SELECT COUNT(c.codigo_cliente) AS numero_clientes, o.ciudad
FROM oficina o
INNER JOIN empleado e
...
SELECT COUNT(c.codigo_cliente) AS numero_clientes_madrid, o.ciudad AS ubicacion_oficina
FROM oficina o
INNER JOIN empleado e
ON o.codigo_oficina = e.codigo_oficina
RIGHT JOIN cliente c
...
SELECT COUNT(c.codigo_cliente) AS numero_clientes, o.ciudad AS ubicacion_oficina
FROM oficina o
INNER JOIN empleado e
ON o.codigo_oficina = e.codigo_oficina
RIGHT JOIN cliente c
...
SELECT MIN(p.fecha_pago) AS primer_pago, MAX(p.fecha_pago) AS ultimo_pago, c.nombre_cliente
FROM pago p
LEFT JOIN cliente c
...
SELECT DISTINCT(pr.nombre) AS producto, COUNT(pe.codigo_pedido) AS cantidad
FROM pedido pe
INNER JOIN detalle_pedido dp
ON pe.codigo_pedido = dp.codigo_pedido
...
SELECT SUM(p.total) AS total_por_pedido, pe.codigo_pedido
FROM pago p
INNER JOIN cliente c
ON p.codigo_cliente = c.codigo_cliente
...
SELECT pr.nombre, SUM(dp.cantidad * pr.precio_venta * 21 / 100) AS iva_venta, SUM(dp.cantidad * pr.precio_proveedor * 21 / 100) AS iva_proveedor
FROM producto pr
INNER JOIN detalle_pedido dp
ON pr.codigo_producto = dp.codigo_producto
GROUP BY pr.nombre;
...
SELECT pr.codigo_producto, pr.nombre, SUM(dp.cantidad * pr.precio_venta * 21 / 100) AS iva_venta, SUM(dp.cantidad * pr.precio_proveedor * 21 / 100) AS iva_proveedor
FROM producto pr
INNER JOIN detalle_pedido dp
ON pr.codigo_producto = dp.codigo_producto
GROUP BY pr.codigo_producto
ORDER BY pr.codigo_producto ASC;

-- 17. La misma información que en la pregunta anterior, pero agrupada por código de producto filtrada por
-- los códigos que empiecen por OR.
...
SELECT pr.nombre, SUM(dp.cantidad * pr.precio_venta * 21 / 100 + p.total) AS con_iva
FROM producto pr
INNER JOIN detalle_pedido dp
ON pr.codigo_producto = dp.codigo_producto
INNER JOIN pedido pe
...
ON c.codigo_cliente = p.codigo_cliente
GROUP BY pr.nombre
HAVING SUM(dp.cantidad * pr.precio_venta * 21 / 100 + p.total) >= 3000;

-- 19. Muestre la suma total de todos los pagos que se realizaron para cada uno de los años que aparecen en la
-- tabla pagos.

SELECT SUM(total) AS suma_total, YEAR(fecha_pago) AS año
FROM pago
GROUP BY año;